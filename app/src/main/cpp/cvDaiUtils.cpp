...
extern "C" void
readModelFromAsset(const char *model_path, std::vector<uint8_t> &model_buf, JNIEnv *env,
                   jobject obj) {
...
cv::Mat getFrame(const std::shared_ptr<dai::ImgFrame> &imgFrame) {
    ...
    if (actualSize < requiredSize) {
        throw std::runtime_error(
                ...
                std::to_string(requiredSize) + ", actual "
...
extern "C" void draw_detections(cv::Mat frame, std::vector<dai::ImgDetection> &detections) {
    ...
    for (auto &detection: detections) {
        ...
        log("Detection: %s [%f,%f,%f,%f], %f", labelStr.c_str(), detection.xmin, detection.ymin,
            detection.xmax, detection.ymax, detection.confidence);

        cv::putText(frame, labelStr, cv::Point(x1 + 10, y1 + 20), cv::FONT_HERSHEY_TRIPLEX, 0.5,
                    color);
        std::stringstream confStr;
        confStr << std::fixed << std::setprecision(2) << detection.confidence * 100;
        cv::putText(frame, confStr.str(), cv::Point(x1 + 10, y1 + 40), cv::FONT_HERSHEY_TRIPLEX,
                    0.5, color);
        cv::rectangle(frame, cv::Rect(cv::Point(x1, y1), cv::Point(x2, y2)), color,
